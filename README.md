...
The `basePath` and `assetPrefix` come from `GITHUB_REPOSITORY` during CI (falling back to `NEXT_PUBLIC_BASE_PATH` or `/gh-poc` locally), so assets and routes resolve correctly under the repository name when the site is hosted on GitHub Pages.
...
3. The site will be available at `https://<username>.github.io/gh-poc/` (or the repository you forked to) with trailing slashes on routes to match the static export configuration.
